# EKS Module: IRSA (IAM Roles for Service Accounts)
# AWS Load Balancer Controller, ExternalDNS 등을 위한 IRSA
...
locals {
  ...
  oidc_provider_url = var.enable_irsa ? aws_iam_openid_connect_provider.cluster[0].url : ""
}
...
resource "aws_iam_role" "alb_controller" {
  count = var.enable_alb_controller_irsa && var.enable_irsa ? 1 : 0
...
# ALB Controller 정책 (AWS 공식 정책 사용)
...
resource "aws_iam_role" "external_dns" {
  count = var.enable_external_dns_irsa && var.enable_irsa ? 1 : 0
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${local.cluster_oidc_issuer}:aud" = "sts.amazonaws.com"
            "${local.cluster_oidc_issuer}:sub" = "system:serviceaccount:kube-system:external-dns"
          }
        }
      }
    ]
    ...
    Statement = [
      {
        ...
        Resource = var.external_dns_hosted_zone_id != "" ? [
